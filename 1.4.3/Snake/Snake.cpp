...
int main(int argc,char* argv[],char** env) {
	...
	DebugModeSetting.Seed = (unsigned)time(NULL);
	while(1) {
		SnakeLength=0;
		system("cls");
		short t=StartMenu();
...
void DebugMode(){
	int f=0;
	int a[4][3]={1,22,2,3,22,10,5,22,6,7,22,2};
	bool flag=false;
	while(1){
		if(!flag){
			system("cls");
			...
			gotoXY(21,4);
			cout<<"更改初始长度  \n\n";
			cout<<"当前种子:"<<DebugModeSetting.Seed;
			gotoXY(21,6);
			cout<<"随机种子\n\n";
			gotoXY(21,8);
			cout<<"退出\n\n";
		...
		if(c1!=0&&c1!=224&&c1!=-32) {
			if(c1==13){		//回车.
				...
				if(f==0){
					...
					cout<<"请输入要更改的随机种子:";
					cin>>DebugModeSetting.Seed;
					srand(DebugModeSetting.Seed);
					cout<<"\n更改成功！即将回到Debug设置界面...";
					Sleep(700);
				} else{
					break;
...
void About(){
	...
	cout<<"更新时间:"<<TIME<<endl;
	Sleep(100);
	cout<<"点个Star⑧ QAQ\n仓库地址:gitee.com/Explorer2345/snake\n";
	Sleep(100);
	system("pause");
}
